---
name: ai-dlc-observe
description: "Internal phase of the ai-dlc pipeline — post-deploy observability checks using Honeycomb, health endpoints, and NFR validation. Invoke directly only via /basic-engineering:ai-dlc-observe when explicitly requested by name. For general requests, use basic-engineering:ai-dlc which routes here automatically."
...
- **Always available**: Can be invoked manually via `/basic-engineering:ai-dlc observe`
...
- Suggest investigation steps (specific Honeycomb queries to run)
- Recommend rollback if error rate is critically elevated
