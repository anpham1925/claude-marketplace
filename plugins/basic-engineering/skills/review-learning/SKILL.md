...
description: Analyze the persistent review feedback log for repeated patterns across tickets, then suggest and optionally apply rule/skill/agent improvements. Use when the user wants to learn from past review mistakes, identify recurring issues across PRs, or improve coding rules based on historical feedback. Also auto-triggered by ai-dlc-release when the persistent feedback log crosses the entry threshold. Triggers for "learn from reviews", "what mistakes do we keep making", "improve our rules", "analyze review patterns", "recurring review issues".
argument-hint: '[optional: focus area like "naming", "imports", "tests"]'
model: sonnet
---

Analyze the persistent review feedback log and turn repeated patterns into durable improvements (rules, skills, agents, lint config).

## Persistent Feedback File

The feedback log lives in the **Claude Code project memory directory** — outside the repo, never committed:
...
This file accumulates entries across tickets and sessions. Each entry has a date, ticket, source (verify, pr-review, ci-fix, self-review, etc.), and categorized issues.

## Steps

### 1. Read the Feedback Log

Read the persistent `review-feedback.md` in full. If it doesn't exist or is empty, inform the user:
...
| Workflow/process mistake | This plugin's skill files (`plugins/basic-engineering/skills/`) |
| Agent behavior | This plugin's agent definitions |

### 9. Mark Items as Addressed

After the user confirms a rule improvement, **annotate the original items** in the persistent feedback file that contributed to that pattern:
...
After applying (or declining) all improvements, append a marker to the persistent feedback file:
...
This marker is used by the weekly auto-trigger — the next analysis checks the date of the last marker.

## Auto-Trigger Integration

`ai-dlc-release` prompts the user to run this skill when:
- 7+ days have passed since the last `## Review Learning Analysis` marker (or since the oldest entry if no marker exists)
- The release phase completes successfully

The user must confirm before the analysis runs — it is never auto-invoked silently.

It can also be manually invoked anytime via `/basic-engineering:review-learning`.
