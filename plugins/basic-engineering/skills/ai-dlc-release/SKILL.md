---
name: ai-dlc-release
description: "Internal phase of the ai-dlc pipeline — gets code from branch to merged PR by delegating to the existing ship-n-check pipeline. Invoke directly only via /basic-engineering:ai-dlc-release when explicitly requested by name. For general requests, use basic-engineering:ai-dlc which routes here automatically."
...
1. **Branch & Commit**: invoke `/basic-engineering:ship-branch {ticket}`
2. **Quality Checks**: invoke `/basic-engineering:ship-quality {ticket}`
3. **Simplify**: invoke `/simplify`
4. **Push & PR**: invoke `/basic-engineering:ship-push-pr {ticket}`
5. **CI/CD**: invoke `/basic-engineering:ship-cicd {ticket}`
6. **Staging**: invoke `/basic-engineering:ship-staging {ticket}`
7. **PR Review**: invoke `/basic-engineering:ship-pr-review {ticket}`
...
- `review-feedback.md` — per-ticket copy (persistent copy already saved outside repo)

```bash
git rm -f docs/<identifier>/state.md docs/<identifier>/review-feedback.md
...
After archiving, check the **persistent** review feedback file (see [shared reference](../ai-dlc/reference/shared.md#review-feedback-format) for location).

Check the last `## Review Learning Analysis` marker for its date (or the oldest entry date if no marker exists). Compare against today's date.

- **If 7+ days since last analysis**: Ask the user before proceeding:
  > **{N} review feedback entries** collected across {M} tickets since last analysis ({last analysis date or "never"}). A week has passed — there may be recurring patterns worth turning into rules.
  >
  > Would you like me to run review learning now? (`/basic-engineering:review-learning`)

  Only invoke `/basic-engineering:review-learning` if the user confirms. If declined, move on.
- **If < 7 days**: Skip silently — not enough time has passed to accumulate meaningful patterns.
