---
name: ai-dlc-verify
description: "Internal phase of the ai-dlc pipeline — verifies acceptance criteria, reviews code quality, validates traceability and NFR compliance. Invoke directly only via /basic-engineering:ai-dlc-verify when explicitly requested by name. For general requests, use basic-engineering:ai-dlc which routes here automatically."
...
- [ ] **Error handling**: correct error types, domain errors used properly
...
After completing the code review (Part 4), append a `source: verify` entry to **both** feedback files. See [shared reference](../ai-dlc/reference/shared.md#review-feedback-format) for the dual-write protocol and entry format.
