...
**Sub-gate:** Check off `Categorized: N actionable, N debatable, N informational`.
...
After addressing all comments (whether fixes were needed or not), append a `source: pr-review` entry to **both** feedback files. See [shared reference](../ship-n-check/reference/shared.md#review-feedback-format) for the dual-write protocol and entry format.
