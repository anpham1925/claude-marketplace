...
# Extract the command and output — guard jq/grep with || true since
# PostToolUse payloads may not contain expected fields.
COMMAND=""
OUTPUT=""
STDERR=""
if command -v jq &>/dev/null; then
  COMMAND=$(echo "$INPUT" | jq -r '.tool_input.command // empty' 2>/dev/null) || true
  OUTPUT=$(echo "$INPUT" | jq -r '.tool_output.stdout // empty' 2>/dev/null) || true
  STDERR=$(echo "$INPUT" | jq -r '.tool_output.stderr // empty' 2>/dev/null) || true
else
...
# grep -q returns 1 on no match; inside `if` this doesn't trigger set -e.
ISSUES_FOUND=false
SUGGESTIONS=""

if echo "$COMBINED" | grep -qiE 'formatting|format.*error|style.*error|indent.*error|whitespace' 2>/dev/null; then
  ISSUES_FOUND=true
  SUGGESTIONS="Formatting issues detected in output."
fi

if echo "$COMBINED" | grep -qiE 'prettier.*--write|Run.*prettier|run.*format' 2>/dev/null; then
  ISSUES_FOUND=true
  SUGGESTIONS="${SUGGESTIONS} Output suggests running a formatter (e.g., prettier --write)."
fi

if echo "$COMBINED" | grep -qiE 'eslint.*--fix|Run.*eslint.*fix|auto-?fix' 2>/dev/null; then
  ISSUES_FOUND=true
  SUGGESTIONS="${SUGGESTIONS} Output suggests running eslint --fix."
fi

if echo "$COMBINED" | grep -qiE 'cargo fmt|rustfmt' 2>/dev/null; then
  ISSUES_FOUND=true
  SUGGESTIONS="${SUGGESTIONS} Output suggests running cargo fmt."
fi

if echo "$COMBINED" | grep -qiE 'gofmt|goimports' 2>/dev/null; then
  ISSUES_FOUND=true
  SUGGESTIONS="${SUGGESTIONS} Output suggests running gofmt."
fi

if echo "$COMBINED" | grep -qiE 'ruff format|black|autopep8|isort' 2>/dev/null; then
  ISSUES_FOUND=true
