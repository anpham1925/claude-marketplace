...
2. **Don't assume silently.** If a recommendation depends on an assumption (e.g. "I assume you're on Postgres 14+", "I assume this runs in CI not locally"), state the assumption out loud before acting on it.
...
| About a library / framework | Link to official docs, the library's GitHub repo, a specific version's changelog |
...
- **Hallucinated APIs** — confidently calling `library.someMethod()` without checking it exists. If you're unsure, grep the installed package or fetch the docs.
- **Phantom file references** — citing `src/foo.ts:123` when you haven't actually read that file in this session.
