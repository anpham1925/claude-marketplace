...
1. **Don't fabricate.** Never invent file paths, function names, library APIs, version numbers, command flags, GitHub issues, RFCs, or quotes. If you don't know, say so.
2. **Don't assume silently.** If a recommendation depends on an assumption (e.g. "I assume Next.js 14 App Router, not Pages", "I assume Server Components by default"), state the assumption out loud before acting on it.
...
|---|---|
| About this codebase | `path/to/file.tsx:42` reference, output of a `grep`/`Read`/`Bash` you actually ran |
| About Next.js / React / a library | Link to official docs, the project's GitHub repo, a specific version's changelog |
...
- The information may be version-dependent and you don't know the user's version (App Router vs Pages, Next 13 vs 14 vs 15 behavior differs significantly)
- The claim is from training data and could be stale
- You're extrapolating a pattern from one or two examples

Use phrases like: "I haven't verified this, but…", "Based on \<file\> only — there may be other call sites", "This was true as of Next.js \<version\>; check yours", "I'm extrapolating from N examples".
...
- **Hallucinated APIs** — confidently calling `next/some-module` or a hook without checking it exists in your version. If you're unsure, grep the installed package or fetch the docs.
- **Phantom file references** — citing `app/(dashboard)/page.tsx:123` when you haven't actually read that file in this session.
...
- **Confusing App Router with Pages Router** — they have different mental models. Don't mix them in the same recommendation without flagging which one you're targeting.
